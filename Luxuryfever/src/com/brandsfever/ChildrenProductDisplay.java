...
public class ChildrenProductDisplay extends Fragment {
	ViewGroup _view;
	ListView child_product_list;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (a <= 6) {
			_view = (ViewGroup) inflater.inflate(
					R.layout.phone_children_product_display, null);
		} else if (a >= 7 && a < 9) {
			_view = (ViewGroup) inflater.inflate(
					R.layout.seven_inch_children_product_display, null);
		} else if (a >= 9) {
			_view = (ViewGroup) inflater.inflate(
					R.layout.ten_inch_children_product_display, null);
			System.out.println("in ten inch");
		}
		child_product_list = (ListView) _view.findViewById(R.id.child_product_list);
		...
		if (a <= 6) {
			_padapter = new PhoneAdapter(ctx, ProductDisplay.children_prdt);
			child_product_list.setAdapter(_padapter);
		} else if (a >= 7) {
			_tadapter = new TabAdapter(ctx, ProductDisplay.children_prdt);
			child_product_list.setAdapter(_tadapter);
		}

		child_product_list.setOnScrollListener(new OnScrollListener() {
			@Override
		...
		_scrollup.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				child_product_list.setSelection(0);
				_scrollup.setVisibility(View.GONE);
			}
		});
